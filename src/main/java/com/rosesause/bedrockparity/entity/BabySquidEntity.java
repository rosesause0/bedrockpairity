...
public class BabySquidEntity extends BabyWaterMobEntity {
    ...
    public BabySquidEntity(EntityType<? extends BabyWaterMobEntity> type, World worldIn) {
        super(type, EntityType.SQUID, worldIn);
        this.rand.setSeed(this.getEntityId());
        this.rotationVelocity = 1.0F / (this.rand.nextFloat() + 1.0F) * 0.2F;
    }

    protected void registerGoals() {
        this.goalSelector.addGoal(0, new BabySquidEntity.MoveRandomGoal(this));
        this.goalSelector.addGoal(1, new BabySquidEntity.FleeGoal());
    }

    public static AttributeModifierMap.MutableAttribute BabySquidAttribues() {
    ...
    public static boolean func_223365_b(EntityType<BabySquidEntity> p_223365_0_, IWorld p_223365_1_, SpawnReason reason, BlockPos p_223365_3_, Random p_223365_4_) {
    ...
    class FleeGoal extends Goal {
        ...
        public boolean shouldExecute() {
            LivingEntity livingentity = BabySquidEntity.this.getRevengeTarget();
            if (BabySquidEntity.this.isInWater() && livingentity != null) {
                return BabySquidEntity.this.getDistanceSq(livingentity) < 100.0D;
            } else {
                return false;
            }
        }
        ...
        public void tick() {
            ...
            if (livingentity != null) {
                Vector3d vector3d = new Vector3d(BabySquidEntity.this.getPosX() - livingentity.getPosX(), BabySquidEntity.this.getPosY() - livingentity.getPosY(), BabySquidEntity.this.getPosZ() - livingentity.getPosZ());
                BlockState blockstate = BabySquidEntity.this.world.getBlockState(new BlockPos(BabySquidEntity.this.getPosX() + vector3d.x, BabySquidEntity.this.getPosY() + vector3d.y, BabySquidEntity.this.getPosZ() + vector3d.z));
                FluidState fluidstate = BabySquidEntity.this.world.getFluidState(new BlockPos(BabySquidEntity.this.getPosX() + vector3d.x, BabySquidEntity.this.getPosY() + vector3d.y, BabySquidEntity.this.getPosZ() + vector3d.z));
                if (fluidstate.isTagged(FluidTags.WATER) || blockstate.isAir()) {
                    ...
                    BabySquidEntity.this.setMovementVector((float)vector3d.x / 20.0F, (float)vector3d.y / 20.0F, (float)vector3d.z / 20.0F);
                }

                if (this.tickCounter % 10 == 5) {
                    BabySquidEntity.this.world.addParticle(ParticleTypes.BUBBLE, BabySquidEntity.this.getPosX(), BabySquidEntity.this.getPosY(), BabySquidEntity.this.getPosZ(), 0.0D, 0.0D, 0.0D);
                }
    ...
    class MoveRandomGoal extends Goal {
        private final BabySquidEntity squid;

        public MoveRandomGoal(BabySquidEntity p_i48823_2_) {
